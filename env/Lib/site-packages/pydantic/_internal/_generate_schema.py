...
class GenerateSchema:
    ...
    def _generate_schema_for_type(
        ...
        if metadata_js_function is not None:
            ...
            if metadata_schema:
                ...
                metadata.setdefault("pydantic_js_functions", []).append(
    ...
    def _typed_dict_schema(
        ...
        with self.defs.get_schema_or_ref(typed_dict_cls) as (
            ...
            config: ConfigDict | None = getattr(
                typed_dict_cls, "__pydantic_config__", None
            )
        ...
        if not params:
            ...
            *items_schema, extra_schema = params  # pragma: no cover
            return core_schema.tuple_positional_schema(  # pragma: no cover
                [self.generate_schema(p) for p in items_schema],
                extra_schema=self.generate_schema(extra_schema),
            )
            ...
            return core_schema.tuple_positional_schema(
                [self.generate_schema(p) for p in params]
            )
    ...
    def _computed_field_schema(
        ...
        try:
            return_type = _decorators.get_function_return_type(
                d.func, d.info.return_type, self.types_namespace
            )
        except NameError as e:
            raise PydanticUndefinedAnnotation.from_name_error(e) from e
    ...
    def _prepare_annotations(
        ...
        if prepare is not None:
            source_type, annotations = prepare(
                source_type, tuple(annotations), self.config_wrapper.config_dict
            )
        ...
        while True:
            ...
            if prepare is not None:
                ...
                new_source_type, remaining = prepare(
                    source_type, tuple(remaining), self.config_wrapper.config_dict
                )
